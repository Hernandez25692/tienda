<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto py-6 px-2 sm:px-4 bg-[#f9fafb] min-h-screen">
        ...
        <div class="hidden md:block">
            <table
                class="w-full text-sm text-left border-separate border-spacing-0 mb-8 bg-white rounded-xl shadow overflow-hidden">
                ...
                <tbody>
                    @forelse ($pedidos as $pedido)
                        <tr class="border-t bg-white hover:bg-[#e0e7ff] transition rounded-xl">
                            ...
                            <td class="p-3">
                                <span
                                    class="inline-block bg-[#1e40af] text-white px-3 py-1 rounded-full text-xs font-bold shadow-sm tracking-wide">
                                    🧾 {{ $pedido->codigo }}
                                </span>
                            </td>
                ...
                ">
                            ...
                            @php
                                ...
                                $tieneAbonosSinConfirmar = $pedido->pagos->where('confirmado', false)->count() > 0;
                            @endphp
                            <td class="p-3 font-semibold">
                                ...
                                @if ($tieneAbonosSinConfirmar)
                                    <div
                                        class="mt-2 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-2 rounded flex items-center gap-2 text-xs shadow">
                                        <span class="text-lg">⚠️</span>
                                        <span>Este pedido tiene abonos <strong>sin confirmar</strong>.</span>
                                    </div>
                                @endif
                            </td>
                            <td class="p-3">
                            ...
                            <td class="p-3">
                                <div class="flex gap-2">
                                    ...
                                    <button type="button" onclick="toggleDetalle({{ $pedido->id }})"
                                        class="bg-[#facc15] text-[#1e40af] px-3 py-1 rounded-lg text-xs font-semibold hover:bg-yellow-400 shadow transition focus:outline-none focus:ring-2 focus:ring-[#facc15]">Ver
                                        Detalle</button>
                                </div>
                            </td>
                        </tr>
                                            ...
                                            @foreach ($pedido->productos as $producto)
                                                <tr class="border-t">
                                                    ...
                                                    <td class="p-2">
                                                        @php
                                                            $precioOriginal =
                                                                $producto->precio_venta ?? $producto->precio; // Asumiendo estos campos
                                                            $precioUsado = $producto->pivot->precio_unitario;
                                                        @endphp
                                                        @if ($precioUsado < $precioOriginal)
                                                            <div class="text-red-600 font-bold text-xs">
                                                                L {{ number_format($precioUsado, 2) }}
                                                                <span
                                                                    class="line-through text-gray-400 text-[10px] ml-1">L
                                                                    {{ number_format($precioOriginal, 2) }}</span>
                                                                <span
                                                                    class="ml-1 bg-red-100 text-red-700 px-2 py-0.5 rounded-full text-[10px]">Oferta</span>
                                                            </div>
                                                        @else
                                                            L {{ number_format($precioUsado, 2) }}
                                                        @endif
                ...
                <div class="bg-white rounded-xl shadow p-5 flex flex-col gap-2">
                    <div class="flex justify-between items-center mb-2">
                        <div class="font-bold text-lg text-[#1e40af] flex items-center gap-1">
                            #{{ $pedido->id }}
                            <span
                                class="inline-block bg-[#1e40af] text-white px-2 py-0.5 rounded-full text-xs font-bold ml-2 shadow-sm">🧾
                                {{ $pedido->codigo }}</span>
                        </div>
                        <span
                            class="inline-block px-3 py-1 rounded-full text-xs font-semibold
                    ...
                    <div class="text-sm mb-1"><span class="font-semibold">👤 Cliente:</span>
                        {{ $pedido->user->name }}</div>
                    <div class="text-sm mb-1 flex items-center gap-1"><span class="font-semibold">📅 Fecha:</span>
                        {{ $pedido->created_at->format('d/m/Y') }}</div>
                    <div class="text-sm mb-1"><span class="font-semibold">💰 Total:</span> L
                        {{ number_format($pedido->total, 2) }}</div>
                    <div class="text-xs mb-1 text-green-600"><span class="font-semibold">✅ Abonado:</span> L
                        {{ number_format($abonado, 2) }}</div>
                    <div class="text-xs mb-1 text-red-600"><span class="font-semibold">⛔ Pendiente:</span> L
                        {{ number_format($saldo, 2) }}</div>
                    <div class="text-sm mb-1"><span class="font-semibold">📦 Entrega:</span>
                        {{ $pedido->fecha_entrega_estimada ? \Carbon\Carbon::parse($pedido->fecha_entrega_estimada)->format('d/m/Y') : '-' }}
                    </div>
                    ...
                    <div id="detalle-{{ $pedido->id }}" class="hidden mt-3">
                        <div class="font-semibold mb-2 flex items-center gap-2 text-[#1e40af]">🛒 Detalle del Pedido:
                        </div>
                        <div class="overflow-x-auto">
