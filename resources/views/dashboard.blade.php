<x-app-layout>
    ...
    <div class="py-4 px-2 sm:py-8 sm:px-4 max-w-full sm:max-w-7xl mx-auto">
        @if (auth()->user()->role === 'admin')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
                ...
                <div class="bg-white p-4 sm:p-6 rounded shadow text-center">
                    ...
                    <div class="text-2xl sm:text-3xl font-bold text-blue-600">L {{ number_format($totalIngresos, 2) }}</div>
                </div>
            </div>
            ...
            @if ($productoTop)
                <div class="bg-white p-4 sm:p-6 rounded shadow flex flex-col sm:flex-row items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
                    ...
                    <div class="text-center sm:text-left">
                        ...
                        <div class="text-xs sm:text-sm text-gray-600">Vendidos: {{ $productoTop->total_vendidos }}</div>
                    </div>
                </div>
            @endif
        ...
        @else
            {{-- CLIENTE DASHBOARD --}}
            <div class="bg-gradient-to-r from-indigo-50 to-white p-4 sm:p-6 rounded-xl shadow mb-6 sm:mb-8 flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
                <div class="bg-indigo-100 rounded-full p-2 sm:p-3 mb-2 sm:mb-0">
                    <svg class="w-7 h-7 sm:w-8 sm:h-8 text-indigo-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="text-center sm:text-left">
                    <h3 class="text-xl sm:text-2xl font-bold text-gray-800 mb-1">¡Hola, {{ auth()->user()->name }}!</h3>
                    <p class="text-gray-600 text-sm sm:text-base">Bienvenido a <span class="font-semibold text-indigo-600">EncargaYa</span>. Consulta tus pedidos recientes y descubre productos recomendados para ti.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:gap-8 md:grid-cols-2 mb-8 sm:mb-10">
                ...
                <div>
                    <div class="flex items-center mb-3 sm:mb-4 gap-2">
                        <svg class="w-5 h-5 sm:w-6 sm:h-6 text-amber-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    @if ($misPedidos->count())
                        <div class="space-y-3 sm:space-y-4">
                            @foreach ($misPedidos as $pedido)
                                <div class="bg-white rounded-xl shadow flex flex-col sm:flex-row items-center gap-3 sm:gap-4 p-3 sm:p-4 border hover:shadow-lg transition">
                                    ...
                                    <div class="flex-1 w-full">
                                        <div class="flex flex-col sm:flex-row justify-between items-center sm:items-start">
                                            <span class="font-bold text-indigo-700 text-sm sm:text-base">#{{ $pedido->codigo }}</span>
                                            @php
                                            ...
                                            @endphp
                                            <span class="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-semibold {{ $badge }} mt-2 sm:mt-0">
                                                @if($icon === 'clock')
                    ...
                    @else
                        <div class="bg-white rounded-xl shadow p-4 sm:p-6 text-center text-gray-500">
                            <svg class="w-8 h-8 sm:w-10 sm:h-10 mx-auto mb-2 text-gray-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                            </svg>
                ...
                <div>
                    <div class="flex items-center mb-3 sm:mb-4 gap-2">
                        <svg class="w-5 h-5 sm:w-6 sm:h-6 text-pink-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                        @foreach ($sugerencias as $producto)
                            <a href="{{ route('productos.show', $producto) }}"
                                ...
                                <div class="w-20 h-20 sm:w-24 sm:h-24 mb-2 flex items-center justify-center bg-gray-50 rounded-lg overflow-hidden">
                                ...
                                <div class="text-center w-full">
                                    <div class="text-sm sm:text-base font-semibold text-gray-800 truncate">{{ $producto->nombre }}</div>
                                    <div class="text-yellow-600 font-bold text-xs sm:text-sm mt-1">L {{ number_format($producto->precio_venta, 2) }}</div>
                                </div>
                            </a>
                        @endforeach
                    </div>
