...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col overflow-x-hidden">
        ...
        <div class="max-w-screen-xl mx-auto px-4 py-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="text-center md:text-left">
                    ...
                    <p class="text-base sm:text-lg text-slate-700 opacity-80">Descubre lo mejor de EncargaYa y haz tu pedido fácil y rápido</p>
                </div>
    ...
    <main class="flex-1 w-full max-w-screen-xl mx-auto px-4">
            ...
            @forelse ($productos as $producto)
                <div class="bg-white rounded-xl overflow-hidden shadow hover:shadow-lg transition flex flex-col h-full">
                    @if ($producto->imagenes->first())
                        <div class="product-image-wrapper relative overflow-hidden cursor-zoom-in bg-white aspect-square flex items-center justify-center">
                            <img
                                src="{{ asset('storage/' . $producto->imagenes->first()->ruta) }}"
                                alt="{{ $producto->nombre }}"
                                class="zoomable-image w-full h-full object-contain"
                                loading="lazy"
                    ...
                    <div class="flex flex-col flex-1 px-4 py-3 space-y-2">
                        <h3 class="font-semibold text-base sm:text-lg text-slate-800 truncate">{{ $producto->nombre }}</h3>
                        <p class="font-bold text-amber-700 text-lg sm:text-xl">L {{ number_format($producto->precio_venta, 2) }}</p>
                        @if ($producto->disponible)
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700 w-max">Disponible</span>
                        @else
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700 w-max">Agotado</span>
                        @endif
                    </div>
                </div>
            @empty
        ...
        <div class="cta-section bg-white rounded-xl shadow px-4 py-8 my-8 text-center max-w-2xl mx-auto">
            <h2 class="cta-title text-lg sm:text-xl font-semibold text-slate-800 mb-4">¿Te gustaría hacer tu pedido con nosotros?</h2>
            <a href="{{ route('register') }}"
    ...
    <footer class="bg-slate-800 text-white py-6 mt-auto">
        <div class="max-w-screen-xl mx-auto px-4 text-center">
            <p class="opacity-80 text-xs sm:text-sm">&copy; {{ date('Y') }} EncargaYa. Todos los derechos reservados.</p>
        </div>
    </footer>
</body>
</html>
