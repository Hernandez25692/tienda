            ...
            @foreach ($productos as $producto)
                <div class="bg-white border border-gray-200 rounded-xl shadow group relative flex flex-col p-2 sm:p-3 h-full transition-all duration-200"
                    ...
                    <div class="flex-1 flex flex-col mt-2 min-h-0">
                        ...
                        @if ($ofertaVigente)
                            <div class="mb-1 w-full">
                                <div class="flex flex-wrap items-center gap-1 w-full">
                                    <span class="font-bold text-base sm:text-lg text-red-600">
                                        L {{ number_format($producto->precio_oferta, 2) }}
                                    </span>
                                    <span class="text-xs sm:text-sm text-gray-400 line-through ml-2">
                                        L {{ number_format($producto->precio_venta, 2) }}
                                    </span>
                                    @php
                                        $descuento = round(100 - ($producto->precio_oferta / $producto->precio_venta * 100));
                                    @endphp
                                    <span class="ml-2 text-red-500 font-bold text-[11px] sm:text-xs">
                                        -{{ $descuento }}%
                                    </span>
                                    @if ($producto->oferta_expires_at)
                                        <span class="text-[11px] sm:text-xs text-orange-600 font-semibold countdown-timer ml-2"
                        ...
                        @else
                            <span class="text-indigo-900 font-bold text-sm sm:text-base">
                                L {{ number_format($producto->precio_venta, 2) }}
                            </span>
                        @endif
                    </div>
