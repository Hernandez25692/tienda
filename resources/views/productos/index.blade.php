            ...
            <x-slot name="header">
                <div class="flex items-center justify-between px-2 sm:px-4 py-2 sm:py-4">
                    ...
                    <a href="{{ route('productos.create') }}"
                       class="inline-flex items-center gap-1 bg-yellow-400 hover:bg-yellow-300 text-indigo-900 font-semibold px-2 py-1 sm:px-4 sm:py-2 rounded-lg shadow transition text-xs sm:text-base">
    ...
    <div class="container mx-auto px-2 sm:px-4 bg-gray-100 min-h-screen py-2 sm:py-4">
        ...
        <div x-data="{ open: false, showAdvanced: false }" class="mb-2 sm:mb-4">
            <button id="btn-filtros"
                @click="open = true"
                class="fixed bottom-3 left-1/2 -translate-x-1/2 z-50 sm:static sm:translate-x-0 sm:bottom-auto sm:left-auto flex items-center gap-1 bg-indigo-900 hover:bg-yellow-400 hover:text-indigo-900 text-yellow-400 font-semibold px-4 py-1.5 sm:px-6 sm:py-2 rounded-full shadow transition text-sm focus:outline-none">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <!-- Modal -->
            <div x-show="open" x-transition.opacity class="fixed inset-0 z-40 flex items-center justify-center bg-black bg-opacity-40">
                <div 
                ...
                >
                    <button @click="open = false"
                        class="absolute top-2 right-2 text-gray-400 hover:text-indigo-900 transition">
                    ...
                    <form method="GET" action="{{ route('productos.index') }}" class="flex flex-col gap-2 overflow-y-auto" style="max-height: 70vh;">
                        <div>
                        ...
                        <div class="flex gap-1">
                            <div class="flex-1">
                                <label for="precio_min" class="block text-xs font-semibold text-gray-600 mb-1">Precio mín.</label>
                                <input type="number" step="0.01" name="precio_min" id="precio_min" value="{{ request('precio_min') }}"
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-yellow-400 focus:ring-yellow-400 text-xs px-2 py-1.5">
                            </div>
                            <div class="flex-1">
                                <label for="precio_max" class="block text-xs font-semibold text-gray-600 mb-1">Precio máx.</label>
                                <input type="number" step="0.01" name="precio_max" id="precio_max" value="{{ request('precio_max') }}"
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-yellow-400 focus:ring-yellow-400 text-xs px-2 py-1.5">
                            </div>
                        </div>
                        <!-- Accordion para filtros avanzados -->
                        <div x-data="{ open: false }" class="mt-1">
                            <button type="button" @click="open = !open"
                                class="w-full flex items-center justify-between text-xs font-semibold text-indigo-900 bg-gray-100 rounded-lg px-2 py-1.5 mb-1 focus:outline-none">
                                Más filtros
                        ...
                        <div class="flex flex-col gap-2 mt-2">
                            <button type="submit"
                                class="w-full bg-yellow-400 hover:bg-yellow-300 text-indigo-900 font-bold px-4 py-2 rounded-lg shadow transition text-base">
                                Buscar
                            </button>
                            <a href="{{ route('productos.index') }}"
                                class="w-full bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold px-4 py-2 rounded-lg shadow text-center transition text-base">
                                Limpiar
                            </a>
                        </div>
                    </form>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                function toggleFiltrosBtn() {
                    ...
                    // Detecta si el usuario está cerca del final de la página
        ...
        <!-- Grid de productos compacto y responsivo estilo Amazon -->
        <div 
            id="productos-lista"
        ...
        >
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow group relative flex flex-col p-1 sm:p-3 min-h-0 h-full transition-all duration-200">
                <a href="{{ route('productos.show', $producto->id) }}" class="block rounded-lg overflow-hidden aspect-square bg-gray-100 flex items-center justify-center h-36 sm:h-44">
                ...
                <div class="flex-1 flex flex-col mt-1 sm:mt-3 min-h-0">
                    <h3 class="text-xs sm:text-sm font-bold text-indigo-900 truncate mb-0.5 leading-tight" title="{{ $producto->nombre }}">
                        {{ $producto->nombre }}
                    </h3>
                ...
                <div class="absolute top-2 right-2 flex-col gap-1 hidden sm:flex group-hover:flex z-10">
                    ...
                    class="bg-yellow-400 hover:bg-yellow-300 text-indigo-900 font-bold px-2 py-1 rounded shadow text-xs mb-1 text-center">
                    ...
                    <form action="{{ route('productos.toggleVisibilidad', $producto->id) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold px-2 py-1 rounded text-xs w-full">
                ...
                <a href="{{ route('productos.show', $producto->id) }}" class="sm:hidden absolute inset-0 z-0"></a>
            </div>
            @endforeach
        </div>
        <div id="cargando-productos" class="flex justify-center py-4 hidden">
            <span class="text-gray-500 text-xs sm:text-sm">Cargando más productos...</span>
        </div>
        <div id="fin-productos" class="flex justify-center py-4 hidden">
        ...
        @if ($productos->isEmpty())
            <div class="flex flex-col items-center justify-center py-12 sm:py-16">
                ...
                <h2 class="text-lg sm:text-2xl font-bold text-indigo-900 mb-1 sm:mb-2">¡No hay productos disponibles!</h2>
                ...
                <a href="{{ route('productos.index') }}"
                    class="inline-block bg-yellow-400 hover:bg-yellow-300 text-indigo-900 font-bold px-4 py-2 sm:px-6 sm:py-3 rounded-lg shadow transition text-sm sm:text-base">
                    Explorar catálogo
                </a>
            </div>
        @endif
        ...
        <script>
            // Agrega la clase productos-scroll a la grid en móvil si se usa grid-cols-1 (no en este caso)
            document.addEventListener('DOMContentLoaded', function () {
                // No es necesario porque usamos grid-cols-2 en móvil
            });
        </script>
    </div>
</x-app-layout>
