<x-app-layout>
    <div 
        x-data="{
            ...
            lightbox: false,
            imagenLightbox: '',
            abrirLightbox(src) { this.imagenLightbox = src; this.lightbox = true; },
            cerrarLightbox() { this.lightbox = false; }
        }"
        class="bg-[#f9fafb] min-h-screen py-2 px-0 sm:px-2"
    >
        <div class="max-w-full md:max-w-4xl mx-auto bg-white shadow-xl rounded-none md:rounded-2xl p-2 md:p-6 flex flex-col gap-4 md:gap-8">
            <!-- Galería de imágenes -->
            <div 
            ...
            >
                <div 
                    class="w-full aspect-square bg-gray-100 rounded-xl overflow-hidden border relative group cursor-zoom-in flex items-center justify-center mx-auto max-w-xs md:max-w-md"
                ...
                >
                    ...
                    >
                    <span class="absolute bottom-2 right-2 bg-white/80 text-xs px-2 py-1 rounded text-gray-700 shadow hidden sm:block">
            ...
            <template x-if="lightbox">
                <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/80" x-show="lightbox"
                    ...
                    <img :src="imagenLightbox"
                        class="max-h-[80vh] max-w-[95vw] md:max-h-[90vh] md:max-w-[90vw] rounded shadow-lg border-4 border-white" alt="Imagen ampliada">
            ...
            <div class="flex flex-col gap-2 md:gap-3">
                ...
                <div class="flex items-center gap-2 mb-1">
                    <span class="text-lg md:text-xl font-semibold text-[#facc15]">
            ...
            <form action="{{ route('carrito.agregar') }}" method="POST" class="mt-1 flex flex-col gap-2 sticky bottom-0 bg-white z-10 pt-2 pb-2 md:static md:bg-transparent md:pt-0 md:pb-0">
                @csrf
                <input type="hidden" name="producto_id" value="{{ $producto->id }}">
                <input type="hidden" name="cantidad" value="1">
                <label class="block text-xs md:text-sm font-medium text-gray-700 mb-0.5">Comentario <span class="text-gray-400">(opcional)</span></label>
        ...
        @if (isset($similares) && $similares->count())
            <div class="max-w-full md:max-w-4xl mx-auto mt-6">
                ...
                <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-indigo-200 md:grid md:grid-cols-4 md:gap-4 md:overflow-visible">
                    @foreach ($similares as $sim)
                        <a href="{{ route('productos.show', $sim) }}"
                            ...
                            <div class="text-xs md:text-sm font-semibold text-gray-800 text-center truncate">{{ $sim->nombre }}</div>
